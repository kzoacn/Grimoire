#include <bits/stdc++.h>

#define __advance __attribute__((optimize("O3")))

typedef long long ll;

typedef std::pair<ll, ll> pll;
#define mkpair std::make_pair

const int INF = 0x3f3f3f3f;
const ll LNF = 0x3f3f3f3f3f3f3f3fll;

int n, m, id[55][55], tot;

int head[5105], nxt[1000005], ed, st, sd;
struct Edge {
	int u, v, cap; ll w;
	
	Edge() {}
	Edge(int u, int v, int cap, ll w): u(u), v(v), cap(cap), w(w) {}
} e[1000005];

__advance __inline void AddEdge(int u, int v, int cap, ll w) {
	nxt[ed] = head[u]; head[u] = ed; e[ed++] = Edge(u, v, cap, w);
	nxt[ed] = head[v]; head[v] = ed; e[ed++] = Edge(v, u, 0, -w);
}

std::priority_queue<pll, std::vector<pll>, std::greater<pll> > Q;

ll D[5105], dist[5105], flow, cost;
bool in[5105];

__advance bool modlabel() {
	while (!Q.empty()) Q.pop();
	for (int i = 0; i < tot; ++i)
		if (in[i]) {
			D[i] = 0;
			Q.push(mkpair(0, i));
		}
		else D[i] = LNF;
	while (!Q.empty()) {
		int x = Q.top().first, y = Q.top().second; Q.pop();
		if (y == sd) break;
		if (D[y] < x) continue;
		for (int i = head[y]; ~i; i = nxt[i]) if (e[i].cap) {
			if (x + e[i].w + dist[e[i].v] - dist[y] < D[e[i].v]) {
				D[e[i].v] = x + e[i].w + dist[e[i].v] - dist[y];
				Q.push(mkpair(D[e[i].v], e[i].v));
			}
		}
	}
	if (D[sd] == LNF) return 0;
	for (int i = 0; i < tot; ++i) if (D[i] > D[sd]) dist[i] += D[sd] - D[i];
	return 1;
}

__advance int aug(int p, int limit) {
	if (p == sd) return flow += limit, cost += limit * dist[st], limit;
	in[p] = 1;
	int kk, lim = limit;
	for (int i = head[p]; ~i; i = nxt[i]) if (e[i].cap) {
		if (!in[e[i].v] && dist[e[i].v] + e[i].w == dist[p]) {
			kk = aug(e[i].v, std::min(e[i].cap, lim));
			lim -= kk;
			e[i].cap -= kk;
			e[i ^ 1].cap += kk;
			if (!lim) return in[p] = 0, limit;
		}
	}
	return limit - lim;
}

__advance __inline ll MCF(int oddeven) {
	for (int i = 0; i < tot; ++i) dist[i] = i == sd ? LNF : 0;
	
	while (!Q.empty()) Q.pop();
	Q.push(mkpair(0, sd));
	while (!Q.empty()) {
		int x = Q.top().first, y = Q.top().second; Q.pop();
		if (dist[y] < x) continue;
		for (int i = head[y]; ~i; i = nxt[i]) {
			if (e[i ^ 1].cap && e[i].w + x < dist[e[i].v]) {
				dist[e[i].v] = e[i].w + x;
				Q.push(mkpair(dist[e[i].v], e[i].v));
			}
		}
	}
	flow = cost = 0;
	do {
		do {
			memset(in, 0, sizeof in);
		} while (aug(st, flow));
	} while (modlabel());
	
	std::cerr << flow << " " << n * m + oddeven << std::endl;
	
	if (flow != n * m + oddeven) return -1;
	return cost;
}

__advance int main() {
	int caseCnt; scanf("%d", &caseCnt);
	for (int kase = 1; kase <= caseCnt; ++kase) {
		memset(head, -1, sizeof head); ed = 0;
		
		scanf("%d%d", &n, &m);
		st = 0, sd = 1; int S = 2, T = 3; tot = 4;
		int odd = 0, even = 0;
		for (int i = 1, x; i <= n; ++i)
			for (int j = 1; j <= m; ++j) {
				id[i][j] = tot; tot += 2;
				
				AddEdge(st, id[i][j] + 1, 1, 0);
				AddEdge(id[i][j], sd, 1, 0);
				
				scanf("%d", &x);
				
				if (x) {
					if (x & 1) {
						AddEdge(S, id[i][j], 1, 0);
						++odd;
					}
					else {
						AddEdge(id[i][j] + 1, T, 1, 0);
						++even;
					}
				}
			}
		for (int i = 1, x; i < n; ++i)
			for (int j = 1; j <= m; ++j) {
				scanf("%d", &x);
				
				AddEdge(id[i][j] + 1, id[i + 1][j], 1, x);
				AddEdge(id[i + 1][j] + 1, id[i][j], 1, x);
			}
		for (int i = 1, x; i <= n; ++i)
			for (int j = 1; j < m; ++j) {
				scanf("%d", &x);
				
				AddEdge(id[i][j] + 1, id[i][j + 1], 1, x);
				AddEdge(id[i][j + 1] + 1, id[i][j], 1, x);
			}
		if (odd != even) {
			printf("Case #%d: -1\n", kase);
			continue;
		}
		AddEdge(T, S, INF, 0);
		AddEdge(st, S, odd, 0);
		AddEdge(T, sd, odd, 0);
		
		printf("Case #%d: %lld\n", kase, MCF(odd));
	}
	
	return 0;
}
