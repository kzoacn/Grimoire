double getSectorArea(const Point &a, const Point &b, const double &r) { // 求扇形面积
...
std::pair<double, double> getSolution(const double &a, const double &b, const double &c) {
	double delta = b * b - 4.0 * a * c; // 求二次方程ax^2 + bx + c = 0的解
...
double getPointDist(const Point &a, const Point &b) { // 原点到线段AB的距离
	Point d = b - a;
