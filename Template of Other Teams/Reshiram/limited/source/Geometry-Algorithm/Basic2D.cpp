struct point {
	...
	inline point unit() const; // unitize
	inline point negate() const { return point(-x, -y); }
	inline point rot90() const { return point(-y, x); }	// counter - clockwise
...
// Basic 2D operators (e.g. +, -, *, /) has been removed.
struct line {
	point s, t;
...
inline double min_point_to_line(const point &a, const line &b) {
	if (dot(b.s - a, b.t - a) < EPS) return fabs(det(b.s - a, b.t - a) / b.length());
