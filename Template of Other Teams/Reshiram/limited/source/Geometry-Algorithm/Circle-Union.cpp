...
void area(vector<circle> &cir) {
	...
	for (int i = 0; i < n; i++) {
		...
		for (int j = 0; j < n; j++)
			if (i != j) {
				if ((cir[i] == cir[j] && i < j) || (cir[i] != cir[j] && cir[i].radius < cir[j].radius + EPS && (cir[i].center - cir[j].center).length() < fabs(cir[i].radius - cir[j].radius) + EPS)) {
					f[i] = false;
					break;
				}
			}
	...
	for (int i = 0; i < n; i++) {
		...
		for (int j = 0; j < n; j++)
			if (j != i) {
				...
				if (d < sqr(cir[j].radius - cir[i].radius) + EPS) {
					...
					double lambda = 0.5 * (1 + (sqr(cir[i].radius) - sqr(cir[j].radius)) / d);
					point cp(cir[i].center + (cir[j].center - cir[i].center) * lambda);
					point nor((cir[j].center - cir[i].center)._rot90().unit() * (sqrt(sqr(cir[i].radius) - (cp - cir[i].center).norm2())));
					point frm(cp + nor);
					point to(cp - nor);
					psh(atan2((frm - cir[i].center).y, (frm - cir[i].center).x), frm, atan2((to - cir[i].center).y, (to - cir[i].center).x), to);
				}
			}
		...
		for (int j = 0; j + 1 < vec.size(); j++) {
			...
			combine(cnt, area, cir[i].center + point(sin(vec[j + 1].theta) - sin(vec[j].theta), cos(vec[j].theta) - cos(vec[j + 1].theta)) * (1. / area / 3 * cub(cir[i].radius)));
			combine(cnt, -sqr(cir[i].radius) * sin(theta) * 0.5, (cir[i].center + vec[j].p + vec[j + 1].p) / 3.);
			combine(cnt, det(vec[j].p, vec[j + 1].p) * 0.5, (vec[j].p + vec[j + 1].p) / 3.);
		}
	}
}
