int sign(const double & x);// aka DCMP
bool equal(const double & x, const double & y); // aka x == y
struct Point {
	double x, y, z;
	double sqrlen() const {
		return x * x + y * y + z * z;
...
double operator % (const Point & a, const Point & b); // dot operator
Point operator * (const Point & a, const Point & b); // det operator(See Basic 3D)
struct Circle {
	double r;
	Point o;
	Circle(); // Init $o$ and $r$
