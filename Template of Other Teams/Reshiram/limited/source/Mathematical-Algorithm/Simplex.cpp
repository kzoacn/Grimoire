std::vector<double> solve(const std::vector<std::vector<double> > &a,
	...
	for (double number; ; ) {
		...
		for (int i = 0; i < n; ++i) {
			if (value[i][s] < -eps) {
				if (r < 0 || (number = value[r][m] / value[r][s] - value[i][m] / value[i][s]) < -eps || number < eps && index[r + m] > index[i + m]) {
					 r = i;
				}
			}
		}
		if (r < 0) return std::vector<double>(); //	Solution is unbounded.
	}
	if (value[n + 1][m] < -eps) return std::vector<double>(); // No solution.
