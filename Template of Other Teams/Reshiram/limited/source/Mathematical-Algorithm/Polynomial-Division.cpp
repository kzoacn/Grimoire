void divide(int n, int m, int *a, int *b, int *d, int *r) { // $n$、$m$分别为多项式$A$（被除数）和$B$（除数）的次数界
	...
	meminit(inv, n - m + 1, M); DFT(inv, M, 0);
	meminit(tA, n - m + 1, M); DFT(tA, M, 0);
	for (int i = 0; i < M; i++) d[i] = 1ll * inv[i] * tA[i] % MOD;
	DFT(d, M, 1);
	std::reverse(d, d + n - m + 1);
	for (M = 1; M <= n; M <<= 1);
	memcopy(tB, b, 0, m); meminit(tB, m, M);
	memcopy(tD, d, 0, n - m + 1); meminit(tD, n - m + 1, M);
	DFT(tD, M, 0); DFT(tB, M, 0);
	for (int i = 0; i < M; i++) r[i] = 1ll * tD[i] * tB[i] % MOD;
	DFT(r, M, 1);
	meminit(r, n, M);
