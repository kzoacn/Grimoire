void getDiff(int *a, int *b, int n) { // 多项式取微分
	for (int i = 0; i + 1 < n; i++) b[i] = 1ll * (i + 1) * a[i + 1] % MOD;
	b[n - 1] = 0;
}
...
void getLn(int *a, int *b, int n) {
	...
	int M = 1;
	for (; M <= 2 * (n - 1); M <<= 1);
	getInv(a, inv, n);　getDiff(a, d, n);
	meminit(d, n, M); meminit(inv, n, M);
	DFT(d, M, 0); DFT(inv, M, 0);
	for (int i = 0; i < M; i++) d[i] = 1ll * d[i] * inv[i] % MOD;
	DFT(d, M, 1);
	getInt(d, b, n);
}
void getExp(int *a, int *b, int n) {
	...
	for (int c = 2, M = 1; c < (n << 1); c <<= 1) {
		...
		meminit(tmp, bound, M); meminit(b, c, M);
		getLn(b, ln, c);
		meminit(ln, c, M);
		DFT(b, M, 0); DFT(tmp, M, 0); DFT(ln, M, 0);
		for (int i = 0; i < M; i++) b[i] = 1ll * b[i] * (1ll - ln[i] + tmp[i] + MOD) % MOD;
		DFT(b, M, 1);
		meminit(b, c, M);
	}
}
