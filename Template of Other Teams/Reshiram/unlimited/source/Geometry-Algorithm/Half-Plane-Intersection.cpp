...
vector<point> cut(const vector<point> &c, line p) {
	...
	for (int i = 0; i < (int)c.size(); ++i) {
		...
		if (two_side(c[i], c[j], p))
			ret.push_back(line_intersubsection(p, line(c[i], c[j])));
	}
	return ret;
}
...
vector<line> half_plane_intersect(const vector<line> &h) {
	...
	while (rear - fore > 1 &&
				 !turn_left(ret[fore], line_intersect(ret[rear - 1], ret[rear])))
		--rear;
	while (rear - fore > 1 &&
				 !turn_left(ret[rear], line_intersect(ret[fore], ret[fore + 1])))
		++fore;
