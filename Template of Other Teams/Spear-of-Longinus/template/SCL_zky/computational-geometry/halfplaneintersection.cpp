...
struct L{
	...
	L push() const{ // 将半平面向外推 eps
		const double eps = 1e-10;
		P delta = (b - a).turn90().norm() * eps;
		return L(a - delta, b - delta);
	}	
};
bool sameDir(const L &l0, const L &l1) { return parallel(l0, l1) && sgn((l0.b - l0.a)^(l1.b - l1.a)) == 1; }
...
bool check(const L &u, const L &v, const L &w) { return w.onLeft(intersect(u, v)); }
