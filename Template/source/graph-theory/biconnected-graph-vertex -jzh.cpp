...
struct BCC {  // N = N0 + M0. Remember to call init(&raw_graph).
	...
	int dfn[N], DFN, low[N];
	int stack[N], top;
	int expand_to[M];  	// Where edge i is expanded to in expaned graph.
	// Vertex i expaned to i.
	int compress_to[N];  // Where vertex i is compressed to.
	bool cut[N], compress_cut[N], branch[M], vis[N], flag;
	...
	void solve() {
		...
		memset(vis + g -> base, 0, sizeof(*vis) * n);
